...
def run():
    with sync_playwright() as p:
        ...
        # Select the 'fw_state' key
        page.get_by_label("Timeseries Key").click()
        page.get_by_role("option", name="fw_state").click()

        # Click the "Fetch" button
        page.get_by_role("button", name="Fetch").click()

        # Verify that the "No data available" message is displayed
        expect(page.locator("text=No data available")).to_be_visible(timeout=30000)

        # Take a screenshot
        page.screenshot(path="jules-scratch/verification/loop_and_data_handling.png")
